...
const ListTeacherinAdmin = () => {
  ...
  const handleGetListUser = async () => {
    try {
      const response = await apiService.get(`lecturers`);
  ...
  const hanleSearchData = (value) => {
    setSearch(value);
    const filteredData = listTeacher.filter(
      (item) =>
  ...
  const handleChangeSort = (value) => {
    ...
    const sortedData = [...fillter].sort((a, b) => {
      if (value === "asc") {
        return a.lecturer_id.localeCompare(b.lecturer_id); // Tăng dần
      }
      return b.lecturer_id.localeCompare(a.lecturer_id); // Giảm dần
    });
...
export default ListTeacherinAdmin;
